...
def garbage(line):
    return not line.startswith('<unknown program name>') and not line.startswith('profiling:')


def memcheck(valgrind, test, test_path):
    os.system(
        valgrind + f' --tool=memcheck --child-silent-after-fork=yes --leak-check=full --xml=yes --xml-fd=3 --num-callers=50 {test_path} 1>{test}.txt 2>{test}.err 3>{test}.xml')
...
if __name__ == '__main__':
    if len(sys.argv) > 2:
        ...
        with multiprocessing.Pool(multiprocessing.cpu_count()) as process:
            results = process.starmap(run_test, [(valgrind, test, os.path.join(tests_dir, test)) for test in tests])
